...
export default function getTerm(y, m, d) {

    for (let i = solarTerm.length, x; i--;) {
        x = term(y, i);
        if (x.m === m && x.d === d) return solarTerm[i];
    }
    return null;
}

function term(y, n) {
    let d = new Date((31556925974.7 * (y - 1900) + sTermInfo[n] * 60000) + Date.UTC(1900, 0, 6, 2, 5));
